...
class WeatherManageBloc extends Bloc<WeatherEvent, WeatherState> {
  ...
  Stream<WeatherState> mapEventToState(WeatherEvent event) async* {
    if (event is CatchCityDataFromSearchBar) {
      yield SearchingState();
      try {
        final weather = await _getWeatherData(event.cityName);
  ...
  Future<WeatherData> _getWeatherData(String cityName) async {
  ...
  WeatherData parseJson(final response) {
